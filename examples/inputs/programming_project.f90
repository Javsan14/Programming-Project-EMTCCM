program geom_opt
    ...
    real*8, allocatable :: stretch_G(:), bend_G(:), torsion_G(:), LJ_G(:)
    real*8, allocatable :: total_G(:), new_total_G(:), total_G_int(:), new_total_G_int(:)
    ...
    ! With the internal coordinates, now we can allocate all the necessary matrices
    ...
    ! Also, before entering the BFGS algorithm, we need to calculate the energy and gradient for the input structure.
    ! Remember that we have all except the LJ component of the energy from the internal coordinates calculation.
    ! - Bond lengths and stretching energy
    ...
    ! Compute Wilson's B matrix and its inverse G matrix for the transformation to internal coordinates
    ...
    do while (rms_grad .ge. threshold)
        ...
        ! 1. Compute search direction using the inverse Hessian
        p_vec = -1.0d0 * matmul(inv_hess,total_G_int)

        ! 2. Step size control: Limit the RMS of the step vector to 0.02
        s_vec = p_vec
        ...
        ! 3. Convert updated internal coordinates back to Cartesian coordinates iteratively
        ...
        ! 4. Recalculate step vector based on the final optimal internal coordinates
        ...
        ! 5. Calculate updated energies and gradients for the new coordinates
        call get_bond_lenghts_and_stretching(coord_mat, symbol, list_bonds, nbonds, bond_lengths, total_stretch_E)
        call get_bending_terms_and_energy(coord_mat, symbol, list_bonds, nbonds, &
        bending_terms, angle_values, nangles, total_bend_E)
        call get_dihedral_terms_and_energy(coord_mat, symbol, bending_terms, nangles, &
        ...
        new_total_G = stretch_G + bend_G + torsion_G + LJ_G

        call get_wilson_b_mat(coord_mat, natoms, nq, nbonds, list_bonds, bond_lengths, &
        nangles, bending_terms, ndihedrals, dihedral_terms, B_mat, G_inv_mat)
        new_total_G_int = matmul(matmul(G_inv_mat, B_mat), new_total_G)

        ! 6. Compute the y vector (change in gradients in internal coordinates)
        y_vec = new_total_G_int - total_G_int

        ! 7. Update the inverse Hessian
        ...
        ! 8. Compute convergence criteria and print output info
        call get_rms(3*natoms, new_total_G, rms_grad) ! Recalculate RMS gradient for convergence check
        ...
        ! 9. Update variables for the next step of the optimization
        coord = new_coord
    ...
    deallocate(stretch_G, bend_G, torsion_G, LJ_G, total_G, new_total_G, total_G_int, new_total_G_int)
...
contains ! Here is where the magic is done
